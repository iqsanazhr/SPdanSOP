...
router.put('/:id/components', async (req, res) => {
  try {
    ...
    await prisma.$transaction(
      components.map((comp) =>
        prisma.documentComponent.update({
          where: { id: comp.id },
          data: {
...
router.post('/:id/components', async (req, res) => {
  try {
    ...
    const targetOrder =
      insertAfterOrder !== undefined ? insertAfterOrder + 1 : document.components.length + 1;
...
router.post('/:id/duplicate', async (req, res) => {
  try {
    ...
    const duplicatedDoc = await prisma.document.create({
      data: {
